...
## 1. Model Training Results (Colab Notebook — `videos4_15frames (1).ipynb`)
...
| 4 | Sorry | Ø¢Ø³Ù |
| 5 | I am fine | أنا بخير |
| 6 | I am sorry | Ø£Ù†Ø§ Ø¢Ø³Ù |
| 7 | Good bye | مع السلامة |
| 8 | Salam aleikum | السلام عليكم |
| 9 | Good morning | صباح الخير |
| 10 | Not bad | ليس سيئاً |
...
| Augmented samples | 540 total (3× augmentation) |
...
| D. Hand Openness Ratios | 5 | Tip-to-wrist distance normalized by palm size (landmark 0 → landmark 9) |
...
|-----------------------|--------|---------|
| Temporal Warping | Speed variations (0.8â€“1.2×) via linear interpolation and resampling | Simulates signing at different speeds |
| Spatial Scaling | Random scaling (0.9â€“1.1×) of all landmark values | Handles variations in hand distance from camera |
| Mirror Augmentation | Horizontal flip of x-coordinates (50% probability) | Simulates left/right hand orientation |
| Hand Tremor | Sinusoidal noise (σ ≈ 0.01) | Simulates natural hand vibration |
| Frame Dropout | Random zeroing of 3 frames (30% probability) | Simulates motion blur and detection gaps |
| Spatial Translation | Random Gaussian translation (σ ≈ 0.02) | Handles off-centre hand positioning |

**Table 3a:** Data augmentation techniques applied during training.

The augmented dataset of 540 sequences was split 80/20 → **432 training, 108 validation** samples. Class balance was maintained: each of the 12 classes received exactly **45 samples** (15 original × 3 augmented versions).
...
- Trained on 6 easy gesture classes: "Good bye", "Good morning", "Good evening", "Salam aleikum", "Thanks", "Sorry" → **216 samples**
- Batch size ≈ 8 (27 batches/epoch)
...
- Fine-tuned on all 12 classes → **432 training samples**
...
- `ReduceLROnPlateau` reduced learning rate from 0.0005 → 0.00025 around epoch 39
...
- **Epoch 0:** Thanks→Alhamdulillah, How are you→Good bye, Good evening→Not bad
- **Epoch 10:** How are you→I am sorry, Good evening→Good morning, Alhamdulillah→Thanks
- **Epoch 20:** Alhamdulillah→Thanks, I am fine↔I am sorry, Salam aleikum→Thanks
- **Epoch 30:** Only 4 remaining confusions: Alhamdulillah→Good evening, I am sorry→I am fine, Salam aleikum→I am fine, I am pleased to meet you→I am sorry
...
**Architecture 1 — Sequential LSTM / GRU (Cell 8)**
...
Note: Input shape is (15, 84) because the raw extraction produces 21 landmarks × 4 values = 84 features. GRU variant has 246,412 parameters (0.96 MB).

**Architecture 2 — Improved Sequential Model (Cell 11)**
...
**Architecture 3 — Hierarchical Functional Model (Cells 17â€“18)**
...
**16. Confusion Matrix**
A 12×12 matrix $C$ where $C_{ij}$ = number of class-$i$ instances predicted as class $j$. Used during training via the `ConfusionMatrixLogger` callback to monitor which class pairs remain confused.
...
**Table 4a:** TTA vs standard prediction on 5 validation samples. Both approaches achieved 100% classification accuracy on this sample; TTA notably improved the low-confidence "I am sorry" prediction (53.59% → 74.12%) but reduced confidence in the high-confidence cases.

---

### 1.4 Per-Class Performance Analysis (Improved Model — 80.56% Accuracy)
...
- **"Sorry"** (precision=0.50): Predicted as "Sorry" when gesture was actually something else — classic confusion with "I am sorry."
...
- **Perfect classes** (F1=1.00): Good evening, Alhamdulillah, I am fine, I am sorry, Good morning, Not bad — these have distinctive hand shapes that the model separates cleanly.
...
| Frame throttling (10 FPS target) | — | Frames arriving faster than 100ms are discarded |
| Base64 encoding + HTTP POST | 40â€“70 | JPEG quality=60, image resized to 320×240 |
| Flask request parsing | 5â€“10 | JSON decode + base64 decode |
| Aspect-ratio letterboxing | 2â€“5 | 9:16 portrait → 420:320 training ratio padding |
| MediaPipe IPC (pipe to mp_worker) | 60â€“150 | `model_complexity=0`; persistent reader thread |
| Feature extraction (mp_worker) | 20â€“40 | 21 landmarks → 100 features |
...
The sliding window strategy — where the oldest frame is dropped rather than clearing the entire buffer on a sub-threshold prediction — reduces the time penalty for ambiguous gestures.
...
A critical finding during integration testing was the impact of the mobile camera's native aspect ratio on hand landmark coordinates. The Colab training data was captured at approximately **420×320 pixels (4:3 aspect ratio)**, while the Flutter camera stream uses portrait mode (~480×640, 9:16 aspect ratio).
...
| Video stream quality at 4G/LTE | 640×480 @ 15â€“30 FPS |
| Video stream quality on local Wi-Fi | 640×480 @ 30 FPS |
...
- `"تلذلذلذلذلذ..."` ✗ (repeated syllables during silence)
- `"3 2 3 2 3 2..."` ✗ (repeated digits during low-signal audio)
...
**Generation 1 — LSTM (94.44%) and GRU (11.11%) on 180 samples:**
...
**Generation 2 — Improved Model with Augmentation (80.56%):**
Applying augmentation (180 → 540 samples) and an enhanced architecture improves GRU-style training to 80.56%, an absolute gain of +41.67% over the 38.89% baseline. This demonstrates that data quantity has a decisive impact when models are trained from scratch on limited data.

**Generation 3 — Hierarchical Model with Curriculum Learning (92.59%):**
...
1. **Inter-class Similarity:** The confusion log at epoch 30 shows the persisting pairs: (I am sorry ↔ I am fine), (Alhamdulillah → Good evening), (I am pleased to meet you → I am sorry). These gestures share overlapping hand configurations.

2. **Small Dataset:** Only 15 original samples per class were used. Despite 3× augmentation (45 per class), the augmented samples share the same underlying hand movements.
...
The use of integer labels with sparse crossentropy (rather than one-hot labels with standard crossentropy) has no effect on model performance — both compute identical gradients — but simplifies the data pipeline by avoiding explicit label encoding.
...
1. **Three Progressive Architectures Evaluated:** The notebook documents the full journey from simple LSTM (94.44% on 36 samples) → augmented improved model (80.56% on 36 samples) → hierarchical curriculum model (92.59% on 108 samples), providing clear evidence for each design decision.
...
|-----------|--------|------------------------|---------------------|
| 15 original samples/class | Limited diversity | 3× data augmentation (540 samples) | Collect more varied samples per class |
...
- **3× data augmentation** (temporal warping, spatial scaling, mirroring, tremor, dropout, translation) expanding 180 → 540 training sequences
...
*Arabic Sign Language Real-Time Communication Platform — April 2026.*
